...
extract(File, Options) when is_list(File), is_list(Options) ->
	...
	return_sections(Sections, Bin, EO)
    catch
	throw:Reason ->
	    {error, Reason}
    end.
...
parse_extract_options([H | T], EO) ->
    case H of
	compile_source ->
	    parse_extract_options(T, EO#extract_options{compile_source = true});
	{sections, Sections} when is_list(Sections) ->
	    parse_extract_options(T, EO#extract_options{sections = Sections});
	_ ->
	    throw({badarg, H})
    end;
...
return_sections(S, Bin, #extract_options{sections = undefined}) ->
    {ok, strip_sections([{shebang,  S#sections.shebang},
			 {comment,  S#sections.comment},
			 {emu_args, S#sections.emu_args},
			 {S#sections.type, Bin}])};
return_sections(S, Bin, #extract_options{sections = Names}) ->
    Sections =
	[{shebang,  {shebang,  S#sections.shebang}},
	 {comment,  {comment,  S#sections.comment}},
	 {emu_args, {emu_args, S#sections.emu_args}},
	 {body,     {S#sections.type, Bin}}],
    Pick =
	fun(Name) ->
		case lists:keysearch(Name, 1, Sections) of
		    {value, {_, Section}} -> Section;
		    false -> throw({badarg, {sections, Names}})
		end
	end,
    {ok, lists:map(Pick, Names)}.

strip_sections([{_Tag, undefined} | TagVals]) ->
    strip_sections(TagVals);
strip_sections([{shebang, "#!" ++ Chars} | T]) ->
    Chopped = string:strip(Chars, right, $\n),
    Stripped = string:strip(Chopped, both),
    if
	Stripped =:= ?SHEBANG ->
	    [shebang | strip_sections(T)];
	true ->
	    [{shebang, Stripped} | strip_sections(T)]
    end;
strip_sections([{comment, Chars} | T]) ->
    Chopped = string:strip(Chars, right, $\n),
    Stripped = string:strip(string:strip(Chopped, left, $%), both),
    if
	Stripped =:= ?COMMENT ->
	    [comment | strip_sections(T)];
	true ->
	    [{comment, Stripped} | strip_sections(T)]
    end;
strip_sections([{emu_args, "%%!" ++ Chars} | T]) ->
    Chopped = string:strip(Chars, right, $\n),
    Stripped = string:strip(Chopped, both),
    [{emu_args, Stripped} | strip_sections(T)];
strip_sections([H | T]) ->
    [H | strip_sections(T)];
strip_sections([]) ->
    [].
