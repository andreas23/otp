...
create_and_extract(Config) when is_list(Config) ->
    ...
    [verify_sections(NewFile, FileInfo, B) ||
	B <- Bodies],

    %% Verify the compile_source option
    ?line ok = escript:create(NewFile, [{source, Source}]),
    ?line {ok, [{source, Source}]} = escript:extract(NewFile, []),
    ?line {ok, [{source, BeamCode2}]} =
	escript:extract(NewFile, [compile_source]),
    verify_sections(NewFile, FileInfo, [shebang, comment, {beam, BeamCode2}]),
...
prepare_creation(Base, Config) ->
    ...
    ?line {ok, [shebang, {comment, _}, {emu_args, EmuArg}, {source, Source}]} =
...
verify_sections(File, FileInfo, Sections) ->
    ...
    HasArg = fun(Tag) ->
		     lists:member(Tag, Sections) orelse
		     lists:keymember(Tag, 1, Sections)
	     end,
    ...
    ?line {ok, Bin2} = file:read_file(File),
    ?line Bin = Bin2, % Assert
    ?line {ok, Sections2} = escript:extract(File, []),
    ?line Sections = Sections2, % Assert
    GetName = fun(Section) ->
		      case Section of
			  {shebang, _}    -> shebang;
			  {comment, _}    -> comment;
    ...
    %% Verify the sections option
    RevSections = lists:reverse(Sections),
    RevNames = lists:map(GetName, RevSections),
    MissingNames = [shebang, comment, emu_args] -- RevNames,
    MissingSections = [{Name, undefined} || Name <- MissingNames],
    TooManyNames = RevNames ++ MissingNames ++ lists:reverse(RevNames),
    TooManySections = RevSections ++ MissingSections ++ Sections,
    ?line {ok, Sections4} = escript:extract(File, [{sections, TooManyNames}]),
    io:format("Names: ~p\n", [TooManyNames]),
    io:format("Expected: ~p\n", [TooManySections]),
    io:format("Actual: ~p\n", [Sections4]),
    ?line Sections4 = TooManySections, % Assert
    ok.
